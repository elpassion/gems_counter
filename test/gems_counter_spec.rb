...
describe GemsCounter do
  subject(:gemfiles) { GemsCounter::Counter.new(dir_name) }
  let(:dir_name) { './test/fixtures/gemfiles' }

  describe '.new'
  it 'creates an instance of Counter' do
    expect(gemfiles).to be_an_instance_of(GemsCounter::Counter)
  end

  context 'with directory empty' do
    let!(:dir_name) { './test/fixtures/gem_test' }
  ...
  context 'with no directory' do
    let!(:dir_name) { './test/fixtures/no_dir'}
    it 'raises an ArgumentError' do
      expect { gemfiles }.to raise_error(ArgumentError)
    end
  end
